...
const CloudinaryImage = ({ 
  ...
  if (imageLoaded === null || imageError === null || cloudinaryAvailable === null) {
    return (
  ...
  // Show fallback if Cloudinary is not configured or image has an error
  if (!cloudinaryAvailable || imageError) {
    return (
      <div className={`bg-[#DDA15E] flex flex-col items-center justify-center ${props.width && props.height ? '' : 'h-full w-full'}`} 
        ...
        <p className="text-[#FEFAE0] text-sm mt-2">{!cloudinaryAvailable ? 'Cloudinary tidak terkonfigurasi' : fallbackText}</p>
      </div>
    );
  }
